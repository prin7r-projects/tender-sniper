/**
 * [BRASSMARK_NOWPAYMENTS_IPN] POST /api/webhooks/nowpayments
 *
 ...
 *    captures it.
 *
 * Never trust an unverified payload. Never echo the raw payload.
 */
...
export async function POST(request: Request) {
  ...
  // Stub — when apps/app ships this becomes a DB write to provision the
  // tenant. For now, journalctl is the audit trail. We deliberately do NOT
  // log the full payload because it can carry pay_address-style identifiers.
  // TODO(wave-3): replace this log with a Prisma insert into the orders table.
  console.log(
    `[BRASSMARK_NOWPAYMENTS_IPN] verified=true order_id=${orderId} status=${status} paid=${paid}`
  );
