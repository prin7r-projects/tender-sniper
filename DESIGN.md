...
|------|--------|----------|
| 2026-05-08 | **Wave 2 design refresh — Linear Midnight Command Center alignment verified.** Dark canvas (`#0E1116` lampblack) confirmed in production. Added `canvas-4` (`#23282F`, charcoal-grey-equivalent) to give the brand a Linear-aligned 4-tier surface ramp (canvas → canvas-2 → canvas-3 → canvas-4) for future borders / elevated overlays without disturbing existing card surfaces. Annotated palette comments in `globals.css` to map Brassmark tokens to their Linear analogs (canvas≈pitch-black, canvas-3≈deep-slate, brass≈neon-lime accent slot). Brand identity (brass accent, Source Serif 4 display, Geist body) preserved per `wave2-design-references.json` policy "do not wholesale-clone the reference." Reference: `/Users/keer/projects/prin7r/design-references/linear.md`. **Brass-vs-neon-lime divergence justification:** brass `#C99540` is kept rather than swapped to Linear's neon-lime `#e4f222` because Brassmark's brand essence is Bloomberg-terminal-adjacent + tender-desk masculine professionalism — brass is the audit / heritage / federal-register-seal signal that anchors the trust posture on a procurement product. Neon-lime would read consumer-software, not government-grade. | Wave 2 design refresh agent |
